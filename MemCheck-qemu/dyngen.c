...
int load_object(const char *filename)
{
    ...
    /* read section headers */
    shdr = (struct external_scnhdr *)load_data(fd, sizeof(struct external_filehdr) + fhdr.f_opthdr, fhdr.f_nscns * sizeof(struct external_scnhdr));

    /* read all section data */
    sdata = (uint8_t **)malloc(sizeof(void *) * fhdr.f_nscns);
    memset(sdata, 0, sizeof(void *) * fhdr.f_nscns);

    for(i = 0;i < fhdr.f_nscns; i++) {
        sec = &shdr[i];
        if (!strstart(sec->s_name,  ".bss", &p))
            sdata[i] = (uint8_t *)load_data(fd, sec->s_scnptr, sec->s_size);
    }
    ...
    coff_symtab = (struct external_syment *)load_data(fd, fhdr.f_symptr, fhdr.f_nsyms*SYMESZ);
    ...
    n_strtab = (uint32_t *)load_data(fd, (fhdr.f_symptr + fhdr.f_nsyms*SYMESZ), STRTAB_SIZE);
    strtab = (char *)load_data(fd, (fhdr.f_symptr + fhdr.f_nsyms*SYMESZ), *n_strtab);
	...
	/* set coff symbol */
	symtab = (struct coff_sym *)malloc(sizeof(struct coff_sym) * nb_syms);
    ...
    /* find text relocations, if any */
    sec = &shdr[coff_text_shndx];
    coff_relocs = (struct external_reloc *)load_data(fd, sec->s_relptr, sec->s_nreloc*RELSZ);
    nb_relocs = sec->s_nreloc;

    /* set coff relocation */
    relocs = (struct coff_rel *)malloc(sizeof(struct coff_rel) * nb_relocs);
...
int gen_file(FILE *outfile, int out_type)
{
	...
	fprintf(outfile,
        ...
        for(i = 0, sym = symtab; i < nb_syms; i++, sym++) {
            ...
            name = get_sym_name(sym);
            if (strstart(name, OP_PREFIX, NULL)) {
                fprintf(outfile, "    EXTERN_C void %s();\n", name); 
            }
    ...
    p_start = text + sym->st_value;
    p_end = p_start + sym->st_size;
        ...
        for(i = 0, rel = relocs;i < nb_relocs; i++, rel++) {
			...
			if (ELF64_R_TYPE(rel->r_info) != R_IA64_PCREL21B)
				...
				fprintf(outfile, "    extern char %s;\n",
					sym_name);
#else
                    fprintf(outfile, "EXTERN_C char %s;\n", sym_name);
#endif
                }
            }
        }
