...
static void lsi_queue_command(LSIState *s)
{
    ...
    if (s->queue_len == s->active_commands) {
        s->queue_len++;
        s->queue = (lsi_queue *)realloc(s->queue, s->queue_len * sizeof(lsi_queue));
    }
...
again:
    ...
    case 1: /* IO or Read/Write instruction.  */
        ...
        if (opcode < 5) {
            ...
            data8 = (insn >> 8) & 0xff;
            opcode = (insn >> 27) & 7;
            myoperator = (insn >> 24) & 7;
            DPRINTF("%s reg 0x%x %s data8=0x%02x sfbr=0x%02x%s\n",
                    opcode_names[opcode - 5], reg,
                    operator_names[myoperator], data8, s->sfbr,
                    (insn & (1 << 23)) ? " SFBR" : "");
            op0 = op1 = 0;
            switch (opcode) {
            case 5: /* From SFBR */
...
void *lsi_scsi_init(PCIBus *bus, int devfn)
{
    ...
    s->queue = (lsi_queue *)qemu_malloc(sizeof(lsi_queue));
    s->queue_len = 1;
    s->active_commands = 0;
