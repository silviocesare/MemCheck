...
int kqemu_init(CPUState *env)
{
    ...
    pages_to_flush = (unsigned long *)qemu_vmalloc(KQEMU_MAX_PAGES_TO_FLUSH *
                                  sizeof(unsigned long));
    if (!pages_to_flush)
        goto fail;

    ram_pages_to_update = (unsigned long *)qemu_vmalloc(KQEMU_MAX_RAM_PAGES_TO_UPDATE *
                                       sizeof(unsigned long));
    if (!ram_pages_to_update)
        goto fail;

    modified_ram_pages = (unsigned long *)qemu_vmalloc(KQEMU_MAX_MODIFIED_RAM_PAGES *
                                      sizeof(unsigned long));
    if (!modified_ram_pages)
        goto fail;
    modified_ram_pages_table = (uint8_t *)qemu_mallocz(phys_ram_size >> TARGET_PAGE_BITS);
